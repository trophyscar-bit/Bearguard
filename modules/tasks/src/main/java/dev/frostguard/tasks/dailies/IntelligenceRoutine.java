...
private static final int SURVIVOR_BATCH_LIMIT = 2;
	...
	// Sanity ceiling on the OCR-read full-list
...
// The actual loop the operator caught live doesn't even trip the counter
...
// state across runs; this is deliberately shorter than a full board refresh (the operator's own past
...
@Override
	protected void execute() {
		...
		if (!intelMissionsDetected) {
			...
			// Observed live (troop-slot economy): no missions ⇒ no genuine slot demand. Drop any
		...
		// Observed live (troop-slot economy): Intel has confirmed actionable missions, so publish its
...
private void tryRescheduleFromCooldownFlow() {
		...
		// Clamp the OCR-read refresh to a
		...
		// and pulls the next run forward to claim. Add +1..3 min of randomness (the operator's request) so we
...
private void finalizePostIntelTaskFlow() {
		// Observed live (troop-slot economy): the Intel pass is over — drop its slot claim. release()
...
private void manageRescheduling(boolean anyIntelProcessed, boolean nonBeastIntelProcessed,
		...
		if (!anyIntelProcessed) {
			...
			if (consecutiveNoProgressCycles >= MAX_CONSECUTIVE_NO_PROGRESS_CYCLES) {
				...
				// Observed live caught live watching the app: "intel timer is wrong" -- this branch
	...
	// Observed live caught live watching the app: "that purple is being ignored... it SHOULD
	...
	// first and Fire Beast goes last, exactly as the operator asked -- a Fire Beast that's too strong
	...
	// Observed live caught live watching the app: several recovery paths in this routine press
	...
	// tap away from actually exiting the game mid-automation -- confirmed live, the operator found it
...
private void handleBeast(ImageSearchResultData beast) {
		...
		// smart=false (the operator's Default profile) this was computed from a good OCR read and then
