...
public class TaskRegistrations {
    ...
    private static DelayedTask createTask(TpDailyTaskEnum type, AccountDescriptor profile) {
        return switch (type) {
            ...
            case EVENT_SCHEDULE_SCAN -> new EventScheduleScanRoutine(profile, type);
            case EVENT_FORTRESS_SCAN -> new AllianceFortressScanRoutine(profile, type);
            case EVENT_STATE_CALENDAR_SCAN -> new StateEventCalendarScanRoutine(profile, type);
